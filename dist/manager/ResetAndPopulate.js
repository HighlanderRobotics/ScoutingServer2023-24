    ...
    removeAndAddTables(createTeams, createTournaments, createMatches, createData, createScouters, createPicklist, createMutablePicklist, pickedTeams, createPitScouting, createEPA, createPredictions) {
        return new Promise(function (resolve, reject) {
            Manager.db.serialize(() => {
                ...
                Manager.db.run(`PRAGMA foreign_keys = 0`, ((err) => { if (err) {
                    console.log(`foreign keys ${err}`);
                } }));
                Manager.db.run('DROP TABLE IF EXISTS `teams`', ((err) => { if (err) {
                    console.log(`dropTeams ${err}`);
                } }));
                Manager.db.run(createTeams, ((err) => { if (err) {
                    console.log(`createTeams`);
                } }));
                Manager.db.run('DROP TABLE IF EXISTS `tournaments`', ((err) => { if (err) {
                    console.log(`dropTourney ${err}`);
                } }));
                Manager.db.run(createTournaments, ((err) => { if (err) {
                    console.log(`createTourney ${err}`);
                } }));
                Manager.db.run('DROP TABLE IF EXISTS `matches`', ((err) => { if (err) {
                    console.log(`dropMatches ${err}`);
                } }));
                Manager.db.run(createMatches, ((err) => { if (err) {
                    console.log(`createMatches ${err}`);
                } }));
                Manager.db.run('DROP TABLE IF EXISTS `data`', ((err) => { if (err) {
                    console.log(`dropData ${err}`);
                } }));
                Manager.db.run(createData, ((err) => { if (err) {
                    console.log(`createData ${err}`);
                } }));
                Manager.db.run('DROP TABLE IF EXISTS `sharedPicklists`', ((err) => { if (err) {
                    console.log(`dropPicklists ${err}`);
                } }));
                Manager.db.run(createPicklist, ((err) => { if (err) {
                    console.log(`createPicklists ${err}`);
                } }));
                Manager.db.run('DROP TABLE IF EXISTS `mutablePicklists`', ((err) => { if (err) {
                    console.log(`dropMutablePicklists ${err}`);
                } }));
                Manager.db.run(createMutablePicklist, ((err) => { if (err) {
                    console.log(`createMutablePicklists ${err}`);
                } }));
                Manager.db.run('DROP TABLE IF EXISTS `pickedTeams`', ((err) => { if (err) {
                    console.log(`dropPickedTeams ${err}`);
                } }));
                Manager.db.run(pickedTeams, ((err) => { if (err) {
                    console.log(`createPickedTeams ${err}`);
                } }));
                Manager.db.run('DROP TABLE IF EXISTS `scouters`', ((err) => { if (err) {
                    console.log(`dropScouters ${err}`);
                } }));
                Manager.db.run(createScouters, ((err) => { if (err) {
                    console.log(`createScouters ${err}`);
                } }));
                Manager.db.run('DROP TABLE IF EXISTS `pitScouting`', ((err) => { if (err) {
                    console.log(`dropPitScouting ${err}`);
                } }));
                Manager.db.run(createPitScouting, ((err) => { if (err) {
                    console.log(`createPitScouting ${err}`);
                } }));
                Manager.db.run('DROP TABLE IF EXISTS `epaTable`', ((err) => { if (err) {
                    console.log(`epaTable ${err}`);
                } }));
                Manager.db.run(createEPA, ((err) => { if (err) {
                    console.log(`createEPA ${err}`);
                } }));
                Manager.db.run('DROP TABLE IF EXISTS `predictions`', ((err) => { if (err) {
                    console.log(`predictions ${err}`);
                } }));
                Manager.db.run(createPredictions, ((err) => { if (err) {
                    console.log(`createdPredictions ${err}`);
                } }));
...
//# sourceMappingURL=ResetAndPopulate.js.map